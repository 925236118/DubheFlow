...
- [ ] 2.1 数据库初始化:WAL + `busy_timeout`,主进程独占句柄
- [ ] 2.2 Schema 建表:project / conversation / message / workflow / workflow_revision / run / step_run / model_call / tool_call / approval / artifact / rollback
- [ ] 2.3 迁移机制:版本号 + 迁移脚本,启动时自动 upgrade
- [ ] 2.4 IPC 数据访问层:renderer 全走 IPC,封装为 Repository 模式
- [ ] 2.5 sqlite-vec 扩展接入(为 workflow_vec 检索预留,可先空表)

## Phase 3 — Spec 与原语注册表

- [ ] 3.1 spec TypeScript 类型定义(对齐设计文档 §3)
- [ ] 3.2 内置原语注册表:`collect` / `require` / `approve` / `llm_generate` / `classify` / `extract` / `split` / `fanout` / `join` / `assemble` / `branch` / `loop` / `rollback_to` / `tool_call` / `apply_change` / `godot_check` / `run_test` / `verify`
- [ ] 3.3 服务贡献节点注册:从 manifest 自动加载到注册表
- [ ] 3.4 数据流取值引擎:`{{ }}` 文本插值 + `$ref` 整体引用 + 三类命名空间 + 作用域
- [ ] 3.5 静态校验器:未定义引用 / 作用域越界 / 类型不符 / 循环数据依赖 / 必填缺失 / 有环无上限 / 不可达节点 / 预算缺失 / 审批被绕过
- [ ] 3.6 `input_hash` 计算(引用解析之后)
- [ ] 3.7 内置全局变量表:`PROJECT_PATH` / `SCRIPT_DIR` / `ART_DIR` / `RUN_ID` …

## Phase 4 — 工作流生成(Planner)

- [ ] 4.1 注册表 → Planner 系统提示词自动生成(原语 schema + 可用节点 + 变量清单)
- [ ] 4.2 对话驱动生成:接收任务 → 调 DeepSeek 产出 spec JSON
- [ ] 4.3 输出约束:JSON Schema / GBNF 约束,保证可解析
- [ ] 4.4 生成门禁:静态校验不过 → 打回重生成(带失败原因)
- [ ] 4.5 草稿态 revision 入库

## Phase 5 — 工作流执行器(Interpreter)

- [ ] 5.1 解释器骨架:读 spec → 拓扑排序 → 逐节点执行 → 写 step_run
- [ ] 5.2 检查点:每步 input/output 落库,支持复用(input_hash 一致则跳过)
- [ ] 5.3 节点执行适配器:`openai_chat` / `cli` / `bridge_rpc` / `module`
- [ ] 5.4 `verify` 原语:规则链优先(退出码 / stderr grep / 文件存在 / 断言),模糊情况才调模型
- [ ] 5.5 `rollback_to` 语义:废弃标记 + 分叉结构 + 两层次数限制(节点 max_retries / 全局 max_rollback_rounds)
- [ ] 5.6 `fanout` / `join`:并行分支 + 失败策略(all_success / all_settled / quorum)
- [ ] 5.7 预算闸门:运行前预估 + 超预算暂停询问
- [ ] 5.8 人工审批门禁 `approve`

## Phase 6 — 项目文件快照(git)

- [ ] 6.1 git 操作封装(isomorphic-git 或 spawn git)
- [ ] 6.2 run 启动:建 `agent/run_<id>` 分支 + 记 `vcs_base_commit`
- [ ] 6.3 脏工作区处理:先提交 / 先 stash(需用户确认)
- [ ] 6.4 每步 commit:message 带步骤号 + 打 checkpoint tag
- [ ] 6.5 回退:`git revert` / `git checkout <tag> -- <path>`,打 before-rollback tag
- [ ] 6.6 `git diff` 三重用途:验证证据 / 审批展示 / 回退粒度
- [ ] 6.7 项目检测:非 git 仓库提示初始化;检测 LFS / 大文件

## Phase 7 — 可视化界面

- [ ] 7.1 对话面板:消息流 + 输入框 + 流式渲染
- [ ] 7.2 只读画布:React Flow 渲染 spec(节点状态:等待/运行中/成功/失败/已废弃)
